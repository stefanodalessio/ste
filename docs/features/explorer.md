...
```ts title="quartz/components/ExplorerNode.tsx" {2-7} // sd.changed
export class FileNode {
  ...
  hidden: boolean // if the node is shown in the file tree // sd.changed
...
```
// sd.changed added block below 'till comment
...
```
// sd.changed added block above as mentioned before
